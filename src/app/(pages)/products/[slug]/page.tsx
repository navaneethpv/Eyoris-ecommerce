...
import Link from "next/link"; // added clickable breadcrumb/back link
...
// Dummy product data (should ideally come from an API or a centralized data source)
...
const ProductPage: React.FC = () => {
  ...
  // compute a primary category and a safe slug for linking back to category page
  // prefer category query param (from category listing) if present
  ...
  const prevImage = () => {
    setActiveImageIndex((i) => (product.images && product.images.length ? (i - 1 + product.images.length) % product.images.length : 0));
  };
  const nextImage = () => {
    setActiveImageIndex((i) => (product.images && product.images.length ? (i + 1) % product.images.length : 0));
  };

  const avgRating =
    product.reviews && product.reviews.length ? product.reviews.reduce((acc, r) => acc + r.rating, 0) / product.reviews.length : 0;

  return (
    <div className="max-w-full mx-auto p-6 bg-gray-100 min-h-screen text-black">
        ...
        <div className="px-6 py-8">
              ...
              <div className="mt-4 flex gap-3 overflow-x-auto">
                {product.images?.map((img, idx) => (
                  <button
                    ...
                    className={`flex-none w-20 h-20 rounded-lg overflow-hidden border-2 ${activeImageIndex === idx ? "border-blue-500" : "border-transparent"} shadow-sm`}
            ...
            <div>
              ...
              <div className="mt-4 flex items-center gap-4">
                ...
                {product.originalPrice && (
                  <div className="bg-red-50 text-red-600 text-xs px-2 py-1 rounded">Sale</div>
                )}
              </div>
                      ...
                      {product.colors.map((color, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center justify-center w-10 h-10 rounded-md border ${selectedColor === color ? "ring-2 ring-blue-400 border-transparent" : "border-gray-200"} focus:outline-none`}
                            title={color}
                          >
              ...
              <div className="mt-6 flex items-center gap-4">
                ...
                <button className="px-5 py-3 bg-white border border-gray-200 rounded-md hover:shadow-lg transition" aria-label="Add to wishlist">
                  ♡ Wishlist
                </button>
              </div>
          ...
          <div className="mt-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
              {/* Small cards kept as placeholders */}
