...
export function useBestDeals() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        const response = await axios.get<ApiResponse>('http://localhost:4000/product');
        ...
        if (Array.isArray(raw)) {
          data = raw;
        } else if (raw && Array.isArray((raw as { data?: SampleProductJson[] }).data)) {
          data = (raw as { data: SampleProductJson[] }).data;
        } else if (raw && Array.isArray((raw as { products?: SampleProductJson[] }).products)) {
          data = (raw as { products: SampleProductJson[] }).products;
        } else if (raw && Array.isArray((raw as { items?: SampleProductJson[] }).items)) {
        ...
        // Safely take up to 8 items from the start
        const end = Math.min(8, data.length);
        const processedProducts = data.slice(0, end)
